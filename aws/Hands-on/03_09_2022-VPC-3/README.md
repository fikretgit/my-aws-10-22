...
Connectivitiy bölümü Private olursa on-premise için, farklı resources lar, transit gateway vs.. için kullanılabiliyormuş... biz Public yaptık..!!

Elastic IP allocation ID  : Second Elastic IP (Allocate deyince otomatik bağlıyor. ismi sonradan değiştirilmeli..!!)
```
...
- Go to windows EC2 named 'Windows public', write Private Instance IP address on browser and show them to website with KEN..
...
bir vpc den internete çıkmadan VPC içindeki bir diğer  resource a ulaşmayı sağlıyor EndPoint..!! ancak bir IAM Role tanımlanması gerekiyor. Bastion Host gerekiyor zira çıkyorum ancak geri dönüşte Bastion Host gerekebilir...!!
# STEP 1: Prep

Security Group    : 
    Sec.Group Name : Public-Sec-Group (Bastion Host)
    VPC'si         : clarus-vpc-a olacak..
    Rules          : TCP --- > 22 ---> Anywhere
                     All ICMP IPv4  ---> Anywhere
                     HTTP--------> Anywhere

Security Group    : 
    Sec.Group Name : Private-Sec-Group2 yapmıştık onun Route içine girip Edit Inbound Rule yapıp aşağıdakileri ekliyoruz..!!
...
- Create a bucket of `clarusway-vpc-endpoint-ft` with following properties, 
...
use case : EC2  
Use cases for other AWS services: boş olacak.. dokunmuyoruz  ---> Next : Permission
...
```
aws s3 ls
aws s3 ls clarusway-vpc-endpoint-ft (benimki)
```
...
1. peering ve altta "Delete related route table entries" seçilmeli..

2. Nat Gateway i siliyoruz..

3. End Point i siliyoruz..

4. Elastic IP yi "Release Elastic IP" seçiyoruz..

5. Default a bakıyoruz.. local ve internet gateway aktif.. 
private local olacak.. sıkıntı yok..

6. s3 : önce empty yapıyoruz.. sonra sliyoruz..

7. Instance ları terminate ediyoruz..
