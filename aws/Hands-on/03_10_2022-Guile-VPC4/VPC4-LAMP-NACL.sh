...
*** Secure Copy ile yapabiliriz:
*** scp -i felix.pem felix.pem ec2-user@"Public IP":/home/ec2-user
*** daha güvenli yol bu eval'den.. felix key imizi Bastion Host EC2'ya 
*** kopyalamış olduk.. 
...
# (You can also create a NAT Gateway for outbound connectivity)
    ...
    Sec Group: "NAT-SG"
General Purpose SSD seçilecek..!! ilk çıkan..
...
sudo systemctl status httpd diyerek "active" olduğunu görebiliriz..
...
# 1. Create a private ec2 instance in Private Subnet 1a
    ...
    Sec Group: Allow >>> "SSH and All ICMP-IPv4" Anywhere olacak
...
Compare security groups and network ACLs uzantısı, dökümanı;
https://docs.aws.amazon.com/vpc/latest/userguide/VPC_Security.html#VPC_Security_Comparison

Control traffic to subnets with Network ACLs dökümanı;
...
*** Ephemeral Ports: Bazı Request ler 80,22 vs. her ne kadar girişte izin almış ve girmiş olsa da, outbound başka bir yerden verilebilyor kullanılan sistem tarafından.. **********************************
*** Ephemeral zaten geçici, kısa süreli demek. dolayısıyla Inbound ve Outbound açık olsa bile Maria DB Private instance a ping atamıyoruz. ayar değişikliği gerekiyor.. **********************************
...
Outbound da TCP(6) aralığını 32768 - 65535 olarak ayarlayarak sorunu çözüyoruz..
dökümantasyonu : https://docs.aws.amazon.com/vpc/latest/userguide/vpc-network-acls.html#nacl-ephemeral-ports
...
1. NACL direk silinmez.. bir subnet association olduğu için.. dolayısyla önce subnet association ı uncheck yapıp save changes diyoruz..

2. subnet association kalkınca 1a private subnet direk default a geçti.. şimdi silinebilir..

3. Route Table da girdi yapıldı.. Nat gateway veya Instance üzerinden çıkış yapılmıştı.. Blackhole dönüştü şimdi Nat Gateway silinince..
Nat-Instance Terminate edilip Private Routes da oluşan Blackhole de remove edilebilir..

4. Nat Gatewayiçin açılan Elastic IP yi Release edilmeli.. bu çok önemli çünkü Paralı..!! 

5. Instance ları Terminate ediyoruz..

6. Security Group lar optional.. silince iyi olur..
